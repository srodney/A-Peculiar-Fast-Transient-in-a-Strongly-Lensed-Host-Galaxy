...
\tablecolumns{4}
\tablecaption{Time delay and magnification predictions from lens models \label{tab:LensModelPredictions}}
\tablehead{ 
    \colhead{Model} & \colhead{$\Delta t_{1,2}$} & \colhead{$\Delta t_{1,3}$} & \colhead{$\mu_1$} & \colhead{$\mu_2$} \\
    \colhead{} & \colhead{(days)} & \colhead{(years)} & \colhead{} & \colhead{} \\
} 
\startdata
