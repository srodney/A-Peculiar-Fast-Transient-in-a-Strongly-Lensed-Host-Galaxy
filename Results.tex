...
in Figures \ref{fig:LightCurves}.  The January event, \spockone, rose
to peak luminosity and faded back to quiescence in only $\sim$7 days,
and the August event, \spocktwo, rose and fell in $<20$ days.  The
host galaxy for both \spock events is at a redshift of
$z=1.0054\pm0.0002$ (see Methods section \ref{sec:Observations}).
After dividing the observed rise/fall timescales by ($1+z$) to correct
for cosmic time dilation, we see that both events exhibited a
charactierstic timescale of $\sim$2--5 days in the rest frame.

To interpret the observed light curves and the timing of these two
events, we use \textcolor{red}{five} independently constructed cluster
mass models to determine the effects of gravitational lensing from the
...
transients do not easily match the most common stellar explosive
events (e.g., novae and SNe), a useful starting point for
classification is to examine the phase space of peak luminosity versus
...
Ca-rich SNe \citep{Filippenko:2003,Perets:2011,Kasliwal:2012}, and
Luminous Red Novae \citep[also called intermediate luminosity red
  transients;][]{Munari:2002,Kulkarni:2007,Kasliwal:2011b}.  The
\spock events are incompatible with all of these explosion categories,
...
The dashed rectangle in Figure~\ref{fig:PeakLuminosityDeclineTimeWide}
represents the ``kilonova'' class (also called a ``macronova'' or
``mini-supernova''), which is a theorized category of optical
transient that may be generated by the merger of a neutron star (NS)
