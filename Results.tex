...
in Figure \ref{fig:LightCurves}.  Both events rose to a peak
luminosity of $\sim$0.08 \microjansky and faded back to quiescence in
$<20$ days.  The host galaxy for both \spock events is at a redshift
of $z=1.0054\pm0.0002$ (see Methods section \ref{sec:Spectroscopy}).
After dividing the observed timescales by ($1+z$) to correct
for cosmic time dilation, we see that both events exhibited a
characteristic timescale of $\sim$2--5 days in the rest frame.
...
\subsection{SNe and SN-like Transients}
...
massive star core collapse SNe.  Less well-understood classes are also
...
The primary reason to exclude any SN-like transient model is the
...
difficult to assess the likelihood of such an occurrence
quantitatively, as there are no measured rates of .Ia or kilonovae.
In a study of very fast optical transients with the Pan-STARRS1
survey, \citet{Berger:2013b} derived a limit of $\lesssim0.05$
Mpc$^{-3}$ yr$^{-1}$ for transients reaching $M\approx -14$ mag on a
timescale of $\sim$1 day.  This limit, though several orders of
magnitude higher than the constraints on novae or SNe, is sufficient
to make it exceedingly unlikely that two such transients would appear
in the same galaxy in a single year.  Furthermore, we have observed no
other transients with similar luminosities and light curve shapes in
high-cadence surveys of 5 other Frontier Fields clusters. Indeed, all
other transients detected in the primary HFF survey have been fully
consistent with normal SNe.  Thus, we have no evidence to suggest that
transients of this kind are common enough to be observed twice in a
single galaxy in a single year.
...
($>10$\Msun).  The class is exemplified by prototypical examples such
as P Cygni, $\eta$ Carinae (\etacar), and S Doradus (S-Dor) \citep[for
  recent overviews of the LBV class, see][]{Smith:2011b,
  Kochanek:2012}.  Although most giant LBV eruptions have been
observed to last much longer than the \spock events
\citep{Smith:2011b}, some LBVs have exhibited repeated rapid outbursts
that are broadly consistent with the very fast \spock light
...
The characteristic timescale of such a caustic crossing event would be on
the order of hours or days (see Methods section
...
the location of the lensing critical curve, but in this case the lens
models indicate that there is most likely a single critical curve that
passes roughly mid-way between the two observed \spock locations.

Reasonable modifications of the MACS0416 lens models can allow for the
possibility of multiple critical curves subtending the \spock host
galaxy, so that instead of just two host images (11.1 and 11.2), the
lensed galaxy arc is made up of 3 or more images of the host.  This
situation is disfavored on the grounds that the arc does not exhibit
any gaps or bright features in the surface brightness profile.
Furthermore, if multiple critical curves cross the \spock host galaxy,
it would be impossible to have a single star in the source plane be
...
\spock events.  With these considerations applied, a stellar caustic
crossing could account for at best only one of the \spock events, not
both.
